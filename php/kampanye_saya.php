    ...
    <div class="content-box" id="content">
        ...
        <?php else: ?>
            <?php foreach ($campaigns as $campaign): ?>
                <?php
                // Ambil total komisi yang telah dibayar kepada influencer dari tabel kolaborasi
                $sql_komisi = "SELECT SUM(komisi) as total_komisi FROM kolaborasi WHERE id_campaign = ?";
                $stmt_komisi = $conn->prepare($sql_komisi);
                $stmt_komisi->bind_param("i", $campaign['id']);
                $stmt_komisi->execute();
                $stmt_komisi->bind_result($total_komisi);
                $stmt_komisi->fetch();
                $stmt_komisi->close();
                $total_komisi = $total_komisi ?: 0;

                // Hitung dana escrow untuk kampanye
                ...
                // Hitung sisa dana escrow (dana escrow dikurangi total komisi yang telah dibayar)
                $sisa_dana = $dana_escrow_kampanye - $total_komisi;
                if ($sisa_dana < 0) $sisa_dana = 0;

                // Hitung persentase sisa dana escrow dibandingkan dengan dana escrow untuk kampanye
                $persentase_sisa_dana = $dana_escrow_kampanye > 0 ? ($sisa_dana / $dana_escrow_kampanye * 100) : 0;
                ?>
                <div class="card-trend mb-3" style="border:1px solid #e3eafc; border-radius:10px; padding:18px 20px; margin-bottom:18px; background:#f8fbff;">
                    ...
                    <div style="font-size:0.98em; margin-bottom:6px;">
                        ...
                        <span style="color:#c62828;">Rp<?php echo number_format($total_komisi, 0, ',', '.'); ?></span>
                    </div>
                    <div class="progress" style="height:12px; background:#e3eafc; border-radius:6px; margin-bottom:8px;">
                        <div class="progress-bar" style="width: <?php echo $persentase_sisa_dana; ?>%; background:#1976d2; border-radius:6px;"></div>
                    </div>
